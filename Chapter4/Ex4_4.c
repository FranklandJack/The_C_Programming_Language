...
int main() {
  ...
  while ((type = getop(s)) != EOF) {
    ...
    case '%':
      ...
      break;
    case '\n':
      // The input lines are only sent to stdin when enter is hit,
      // hence the last value to be pushed onto the stack is always
      // popped, so you can only have at most one item on the stack
      // between entering expressions and this means you can't test
      // the added features.
      //printf("\t%.8g\n", pop());
      break;
    case 'c':
